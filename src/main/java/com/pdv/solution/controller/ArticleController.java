...
import com.pdv.solution.repository.ArticleRepository;
...
public class ArticleController {
    ...
    public ResponseEntity<List<Article>> getAllArticles(@RequestParam(required = false) String nom) {
        try {
            ...
            if (nom == null)
                ...
                articleRepository.findArticleByNom(nom).forEach(articles::add);
    ...
    public ResponseEntity<Article> createArticle(@RequestBody Article article) {
        try {
            Article _article = articleRepository
                    .save(new Article(article.getNom(), article.getDescription(), article.getPrice()));
    ...
    public ResponseEntity<Article> updateArticle(@PathVariable("id") int id, @RequestBody Article article) {
        ...
        if (articleData.isPresent()) {
            ...
            _article.setIsAvailable(article.getIsAvailable());
    ...
    public ResponseEntity<List<Article>> findAvailableArticles() {
        try {
            List<Article> articles = articleRepository.findByIsAvailable(true);
